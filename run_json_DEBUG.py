...
#argv=['./run_json.py', '{"feature_dict_list": [{"folder_path": "./data/signals/gemaps_features_processed_50ms/znormalized", "features": ["F0semitoneFrom27.5Hz", "jitterLocal", "F1frequency", "F1bandwidth", "F2frequency", "F3frequency", "Loudness", "shimmerLocaldB", "HNRdBACF", "alphaRatio", "hammarbergIndex", "spectralFlux", "slope0-500", "slope500-1500", "F1amplitudeLogRelF0", "F2amplitudeLogRelF0", "F3amplitudeLogRelF0", "mfcc1", "mfcc2", "mfcc3", "mfcc4"], "modality": "acous", "is_h5_file": false, "uses_master_time_rate": true, "time_step_size": 1, "is_irregular": false, "short_name": "gmaps50"}, {"folder_path": "./data/extracted_Identities/", "features": ["A", "B", "C", "D", "C+S", "dia_num"], "modality": "acous", "is_h5_file": false, "uses_master_time_rate": true, "time_step_size": 1, "is_irregular": false, "short_name": "wrd_reg", "title_string": "_id", "use_glove": false, "glove_embed_table": ""}], "results_dir": "./no_subnets_0/6_Acous_50ms_ADOS_ID/test/", "name_append": "0_6_Acous_50ms_ADOS_ID_m_100_a_0_v_0_lr_01_l2e__l2o_-07_l2m_-05_l2a__l2v__dmo_5_dmi_25_dao__dai__dvo_0_dvi__seq_600", "no_subnets": false, "hidden_nodes_master": 100, "hidden_nodes_acous": 0, "hidden_nodes_visual": 0, "learning_rate": 0.01, "sequence_length": 600, "num_epochs": 1500, "early_stopping": true, "patience": 10, "slow_test": true, "train_list_path": "./data/splits/training_0.txt", "test_list_path": "./data/splits/testing_0.txt", "use_date_str": false, "freeze_glove_embeddings": false, "grad_clip_bool": false, "l2_dict": {"emb": 0, "out": 1e-07, "master": 1e-05, "acous": 0, "visual": 0}, "dropout_dict": {"master_out": 0.5, "master_in": 0.25, "acous_in": 0, "acous_out": 0, "visual_in": 0, "visual_out": 0.0}}']
...
if not (len(argv) == proper_num_args):
    ...
    no_subnets = True
    feature_dict_list = feat_dicts.id_reg_dict_listA 
...
def test():
    ...
    for pause_str in pause_str_list:
        ...
        pause_fscore_MJ=f1_score(true_vals, np.zeros([len(predicted_class)]).tolist(), average='weighted')
    ...
    if onset_test_flag:
        ...
        onset_fscore_MJ=f1_score(true_vals, np.zeros([len(predicted_class)]).tolist(), average='weighted')
