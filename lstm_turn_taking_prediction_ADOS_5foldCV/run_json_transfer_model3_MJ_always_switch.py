...
Model_use=''
if Model_use=='Model1':
    from lstm_model_att_model1 import LSTMPredictor
elif Model_use=='Model2':
    from lstm_model_att_model2 import LSTMPredictor
elif Model_use=='Model3':
    from lstm_model_att_model3 import LSTMPredictor
else:
    from lstm_model_baseline import LSTMPredictor
...
for training_i in range(5):
    ...
    #        loss = loss_func_BCE_Logit(out_test,y_test.transpose(0,1))
        ...
        for pause_str in pause_str_list:
            ...
            print('majority vote f-score(' + pause_str + '):' + str(
                f1_score(true_vals, np.ones([len(predicted_class)]).tolist(), average='weighted')))
        ...
        if onset_test_flag:
            ...
            print('majority vote f-score:' + str(
                f1_score(true_vals_onset, np.ones([len(true_vals_onset), ]).tolist(), average='weighted')))
    ...
    #import glob
    PATH="./model_save/checkpoint{0}.pkl".format(prediction_length)
