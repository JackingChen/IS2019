...
import copy
import pandas as pd
labels=pd.read_excel('./Feature_ADOS.xlsx')
...
# =============================================================================
    ...
    for pause_str in pause_str_list:
        ...
        if pause_str == '50ms':
            eval_frame=1
        elif pause_str == '250ms':
            eval_frame=5
        elif pause_str == '500ms':
            eval_frame=10
        for conv_key in test_file_list:
            ...
            for g_f_key in list(hold_shift[pause_str + '/hold_shift' + '/' + conv_key].keys()):
                ...
                for frame_indx, true_val in hold_shift[pause_str + '/hold_shift' + '/' + conv_key + '/' + g_f_key]:
                    ...
                    if frame_indx < len(results_dict[conv_key + '/' + g_f_key]):
                        ...
                        if results_dict[conv_key + '/' + g_f_key][frame_indx, eval_frame] > results_dict[conv_key + '/' + g_f_key_not[0]][frame_indx, eval_frame]:
#                        if np.sum(
...
#        print('early stopping called at epoch: ' + str(epoch + 1))
    if (epoch == FIX_epoch):
        print(' stopping called at Fixed epoch: ' + str(FIX_epoch))
