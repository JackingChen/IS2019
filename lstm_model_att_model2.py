...
# =============================================================================
'''
    Model2: lstmout=h_ADOSid * h_acous
...
class LSTMPredictor(nn.Module):
    ...
    def __init__(self, lstm_settings_dict, feature_size_dict={'acous': 0, 'visual': 0},
        ...
        # attention
        self.attn = nn.Linear(self.feature_size_dict['visual']+self.lstm_settings_dict['hidden_dims']['acous']*2, self.lstm_settings_dict['hidden_dims']['acous']).type(dtype)
...
# =============================================================================
    ...
    def forward(self, in_data):
        ...
        if not (self.lstm_settings_dict['no_subnets']):
            ...
            for i,x_ in enumerate(x['acous']):
                #acoustic features
                h_tmp, self.hidden_dict['acous'] = self.lstm_dict['acous'](x_.contiguous().view(1,self.batch_size,self.feature_size_dict['acous']), self.hidden_dict['acous'])
                h_tmp=h_tmp.view(1*self.batch_size,self.lstm_settings_dict['hidden_dims']['acous'])
                
   
                acous_hid=torch.cat((self.hidden_dict['acous'][0],self.hidden_dict['acous'][1]),dim=2)
                
                #ADOS ID
                
                ADOS_id_acous_hid=torch.cat((x['visual'][i,:,:],acous_hid.view(1*self.batch_size,-1)),dim=1)                
                h_ADOS_id_acous_hid=self.attn(ADOS_id_acous_hid)
                h_ADOS_id_acous_hid=h_ADOS_id_acous_hid*h_tmp
#                h_ADOS_id_acous_hid=F.softmax(h_ADOS_id_acous_hid)
                
                h_ADOS_id_acous_hid_list.append(h_ADOS_id_acous_hid)

            h_ADOS_id_acous_hid_list=torch.stack(h_ADOS_id_acous_hid_list)
            h['acous']=h_ADOS_id_acous_hid_list
            h['acous'] = self.dropout_dict['acous_out'](h['acous'])
            
            h['visual']=x['visual']
            h['visual'] = self.dropout_dict['visual_out'](h['visual'])
