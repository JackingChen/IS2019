...
# =============================================================================
'''
    Model2: lstmout= lstm['master'](torch.cat(self_attention(acous) * h_acous))
...
class LSTMPredictor(nn.Module):
    ...
    def __init__(self, lstm_settings_dict, feature_size_dict={'acous': 0, 'visual': 0},
                 batch_size=32, seq_length=200, prediction_length=60, embedding_info=[],seq_wind=10,attn_hidden=10):
        ...
        self.attn = nn.LSTM(self.feature_size_dict['acous'],self.attn_hidden,num_layers =self.num_layers,bidirectional=True).type(dtype)
        self.attn2= nn.Linear(2*self.attn_hidden,1).type(dtype)
...
# =============================================================================
        ...
        self.hidden_dict['attn'] = (Variable(
            torch.zeros(self.num_layers*2, self.batch_size, self.attn_hidden)).type(  # biudirectional so self.num_layers*2
            dtype), Variable(torch.zeros(
                self.num_layers*2, self.batch_size,self.attn_hidden)).type(dtype))
# =============================================================================
# 
# =============================================================================
    ...
    def forward(self, in_data):
        ...
        if not (self.lstm_settings_dict['no_subnets']):
            h_ADOS_id_acous_hid_list=[]
#            acous_out_temp=torch.zeros(self.seq_length,self.batch_size,self.lstm_settings_dict['hidden_dims']['acous']).type(dtype)
            h_tmp_front, self.hidden_dict['acous'] = self.lstm_dict['acous'](x['acous'][:self.seq_wind,:,:],self.hidden_dict['acous'])
            for seq_i in range(self.seq_wind,self.seq_length,1):
                #acoustic features
                self.init_hidden()
                h_tmp, self.hidden_dict['acous'] = self.lstm_dict['acous'](x['acous'][seq_i-self.seq_wind:seq_i,:,:],self.hidden_dict['acous'])
#                h_tmp=h_tmp.squeeze(0) # h_tmp -> (self.batch_size,self.feature_size_dict['acous'])
                
                x_resized=x['acous'][seq_i-self.seq_wind:seq_i,:,:].contiguous().view(self.seq_wind,self.batch_size,self.feature_size_dict['acous'])   
                                             
                x_resized, self.hidden_dict['attn']=self.attn(x_resized,self.hidden_dict['attn'])#  
                x_resized=self.attn2(x_resized.squeeze(0))
                x_resized=x_resized.view(self.batch_size,1,self.seq_wind) #(self.seq_wind,self.batch_size,1)                
                attn_weights=F.softmax(x_resized,dim=2)                
                h_tmp_attn = torch.bmm(attn_weights, h_tmp.view(self.batch_size,self.seq_wind,self.lstm_settings_dict['hidden_dims']['acous'])) #(self.batch_size,1,self.seq_wind) * (self.batch_size,self.seq_wind,self.lstm_settings_dict['hidden_dims']['acous']) -> (self.batch_size,1,self.lstm_settings_dict['hidden_dims']['acous'])
                
                h_tmp_attn=h_tmp_attn.squeeze(1)                                
                h_ADOS_id_acous_hid_list.append(h_tmp_attn)

        
            h_ADOS_id_acous_hid_list=torch.stack(h_ADOS_id_acous_hid_list)
            h['acous']=torch.cat((h_tmp_front,h_ADOS_id_acous_hid_list),dim=0)
            h['acous'] = self.dropout_dict['acous_out'](h['acous'])
