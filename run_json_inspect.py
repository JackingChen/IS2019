...
# -*- coding: utf-8 -*-
'''
    In ADOS dataset, overlaps is rare so not running overlap experiments
'''
...
from lstm_model01 import LSTMPredictor
...
if not (len(argv) == proper_num_args):
    ...
    no_subnets = True
    feature_dict_list = feat_dicts.id_reg_dict_list_visual 
...
print("NOW GOING: ",test_list_path)
...
if 'onsets' in locals():
    if isinstance(onsets, h5py.File):  # Just HDF5 files
        try:
            ...
            pass  # Was already closed
if ('overlaps' in locals()) and OVRLPS_bool:
...
#optimizer_list.append(optim.Adam(model.attn.parameters(), lr=learning_rate, weight_decay=0.0))
#optimizer_list.append(optim.Adam(model.attn2.parameters(), lr=learning_rate, weight_decay=0.0))
...
for epoch in range(0, num_epochs):
    ...
    model.eval()
    df_MJ_result=pd.DataFrame([],columns=onset_str_list+pause_str_list)
    # setup results_dict
    ...
    for batch_indx, batch in enumerate(test_dataloader):
        ...
        for file_name, g_f_indx, time_indices, batch_indx in zip(file_name_list,
                                                                 ...
                                                                 time_index_list,
                                                                 range(batch_length)):
#            print("TEST DEBUG",time_indices[0],time_indices[1])
...
#            print("DEBUG: ",pause_str + '/hold_shift' + '/' + conv_key)
        ...
        pause_fscore_MJ=str(f1_score(true_vals, np.zeros([len(predicted_class)]).tolist(), average='weighted'))
        df_MJ_result[pause_str]=pause_fscore_MJ
    ...
    if onset_test_flag:
        ...
        onset_fscore_MJ=str(f1_score(true_vals, np.zeros([len(predicted_class)]).tolist(), average='weighted'))
        df_MJ_result[onset_str_list[0]]=f_score
    ...
    if OVRLPS_bool:
        for overlap_str in overlap_str_list:
            ...
            OVRLPS_fscore_MJ=f1_score(true_vals, np.zeros([len(predicted_class)]).tolist(), average='weighted')
            print('majority vote f-score:' + str(
