...
class LSTMPredictor(nn.Module):
    ...
    def __init__(self, lstm_settings_dict, feature_size_dict={'acous': 0, 'visual': 0},
                 batch_size=32, seq_length=200, prediction_length=60, embedding_info=[],seq_wind=10):
...
# =============================================================================
        if  self.lstm_settings_dict['visual_as_id']['visual'] :
            self.hidden_dict['visual'] = (Variable(
                torch.zeros(self.num_layers, self.batch_size, self.lstm_settings_dict['hidden_dims']['acous'])).type(
                dtype), Variable(torch.zeros(
                    self.num_layers, self.batch_size,self.lstm_settings_dict['hidden_dims']['acous'])).type(dtype))
# =============================================================================
# 
# =============================================================================
...
#                    print("DEBUG, lstm model ",x[mod].shape )
                    if not self.lstm_settings_dict['visual_as_id'][mod]: # if visual modality is not id, then feed it into lstm, else concate it
                        h[mod], self.hidden_dict[mod] = self.lstm_dict[mod](x[mod], self.hidden_dict[mod])
                    else: #  visual modality is  ADOS id
#                        h[mod] = x[mod]
                        self.attn = nn.Linear(self.feature_size_dict['visual'], self.lstm_settings_dict['hidden_dims']['visual']).type(dtype)
                                                                        
                        acous_out_temp=torch.zeros(self.seq_length,self.batch_size,self.lstm_settings_dict['hidden_dims']['acous']).type(dtype)
#                        acous_out_temp[:self.seq_wind,:,:]=h_acous[:self.seq_wind,:,:]    
                        
                        acous_out_temp[:self.seq_wind,:,:], self.hidden_dict[mod]=self.lstm_visual(x['acous'][:self.seq_wind,:,:],self.hidden_dict[mod])
                        
                        for seq_i in range(self.seq_wind,self.seq_length,1):
                            lstm_o=h['acous'][seq_i-self.seq_wind:seq_i,:,:]
#                            lstm_o,self.hidden_dict[mod]=self.lstm_visual(x['acous'][seq_i-self.seq_wind:seq_i,:,:],self.hidden_dict[mod])                                                                                       
                            acous_out_temp[seq_i,:,:]=torch.mean(lstm_o,0,True)
                            
                            
                            x_resized=x['visual'][-1,:,:].contiguous().view(self.batch_size*1,self.feature_size_dict['visual'])
                            x_resized=self.attn(x_resized)
                            x_resized=x_resized.view(1,self.batch_size,self.lstm_settings_dict['hidden_dims']['visual'])
                            
                            attn_weights=x_resized.view(self.batch_size*1,self.lstm_settings_dict['hidden_dims']['visual']) #for the convenience of bmm function
                            attn_weights=F.softmax(attn_weights)
                            attn_weights=attn_weights.view(self.batch_size,1,self.seq_wind)
                            
                            attn_applied=torch.bmm(attn_weights,lstm_o.contiguous().view(self.batch_size,self.seq_wind,self.lstm_settings_dict['hidden_dims']['visual']))
                            
                            
                        h['acous']=acous_out_temp
                        
#                    print("DEBUG, lstm model ",h[mod].shape )
                ...
                # apply dropout
                if not self.lstm_settings_dict['visual_as_id']['visual']:
                    h[mod] = self.dropout_dict[str(mod)+'_out'](h[mod])
    
                    h_list.append(h[mod])
            
            if not self.lstm_settings_dict['visual_as_id']['visual']:# if visual modality is not id, then concat it to acous modality
                ...
                lstm_out=self.dropout_dict[str('acous')+'_out'](h['acous'])
